...
# 分配给20个agent
agents = {}
batch_size = 6  # 每个agent处理6个类别
for i in range(20):
    start = i * batch_size
    end = start + batch_size if i < 19 else len(categories)
...
print(f"Total categories assigned: {sum(len(cats) for cats in agents.values())}")

# 保存
with open("scripts/translation_tasks_20agents.json", "w", encoding="utf-8") as f:
